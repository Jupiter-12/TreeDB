...
CONFIG_LOCK = RLock()
CONFIG = {
    'DB_PATH': str(Path(os.environ.get('DB_PATH', BASE_DIR / 'treedb.sqlite')).resolve()),
...
def update_session_config(session_id, config_payload):
    normalized = normalize_config_payload(config_payload)
    with SESSION_LOCK:
        cleanup_expired_sessions_locked()
        session = SESSIONS.get(session_id)
...
class TreeRequestHandler(SimpleHTTPRequestHandler):
    ...
    def handle_update_config(self, parsed):
        ...
        try:
            if session_id:
                session = update_session_config(session_id, payload)
            else:
